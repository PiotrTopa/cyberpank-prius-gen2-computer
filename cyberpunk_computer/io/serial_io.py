...
class SerialPort(BidirectionalPort):
    ...
    def force_reconnect(self, attempt: int = 1) -> None:
        ...
        if hub_present:
            ...
            # signature of a dead USB host controller ("xHCI HC died"). We
            # DO NOT rebind the host controller here because on SDM845,
            # unbinding DWC3/xHCI cascades into unbinding the DRM display
            # subsystem (due to Type-C DP alt mode device links) and hard
            # hangs the kernel. Instead, we do nothing and let the powerbox
            # heartbeat watchdog force a hardware power-cycle.
            logger.error(
                "Device %s %s AND parent hub %s is gone — USB host controller "
                "likely dead; waiting for hardware watchdog power-cycle.",
                self.config.port, reason, hub_id,
            )
        else:
            logger.error(
